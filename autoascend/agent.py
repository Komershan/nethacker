...
# rest_to_heal: start resting below this HP fraction, stop at the other; hostiles this close stop it
REST_BELOW_HP_RATIO = 0.5
REST_UNTIL_HP_RATIO = 0.9
REST_MONSTER_DISTANCE = 4
...
# emergency flight to a known down staircase: at most this many steps away, with a hostile this close
FLEE_STAIRS_HP_RATIO = 0.5
FLEE_STAIRS_MAX_DIST = 12
FLEE_STAIRS_THREAT_DIST = 5
...
class Agent:
    ...
    def emergency_strategy(self):
        ...
        # hypothesis: the rule above only fires when the character happens to stand on '>', yet the
        # deep runs (which carry most of the score: Dlvl 12 = 0.21, Dlvl 16 = 0.32) die at crisis HP
        # in melee with a known down staircase a few steps away (elves, soldiers, unicorns, mumakil).
        # Below half HP with prayer spent (the prayer rule above had its chance) and a hostile closing
        # in, walk to a '>' within FLEE_STAIRS_MAX_DIST steps and take it: the next level is a fresh start where
        # rest_to_heal can recover, only adjacent monsters follow, and the extra depth is banked
        # even if the fight goes on. One step per call, so the other emergency rules re-evaluate.
        # Not on Dlvl 1: there the grind would only walk back up into the same fight, with no depth banked.
        if self.blstats.hitpoints < FLEE_STAIRS_HP_RATIO * self.blstats.max_hitpoints and \
                self.blstats.depth > 1 and \
                self.current_level().dungeon_number in (Level.DUNGEONS_OF_DOOM, Level.GNOMISH_MINES):
    ...
    # hypothesis: the bot never rests. After a fight it walks on at whatever HP it has left, so the
    # next ordinary monster (gecko, rat, bat, kitten on the Dlvl 1 grind; traces: pri-elf s13 kept
    # grinding at 14/43 HP and died on 9/43) meets a half-dead character, and the emergency rules
    # only fire once HP is already critical. Below half HP with no hostile close by and no hunger
    # emergency, stand on Elbereth and search until HP is back to 90%. Below Xp 10 that costs
    # ~5-15 turns per HP point, cheap next to a 15k-turn grind, and monsters keep spawning and
    # coming to us meanwhile, so experience is barely delayed. Every fight then starts near full HP.
    @utils.debug_log('rest_to_heal')
    @Strategy.wrap
    def rest_to_heal(self):
        hp, max_hp = self.blstats.hitpoints, self.blstats.max_hitpoints
        if hp >= REST_UNTIL_HP_RATIO * max_hp:
            self._resting = False
        elif hp < REST_BELOW_HP_RATIO * max_hp:
            self._resting = True
        if not getattr(self, '_resting', False) or \
                self.blstats.hunger_state >= Hunger.WEAK or \
                self.character.prop.polymorph or \
                self.current_level().shop[self.blstats.y, self.blstats.x]:
            yield False
        for dis, _, _, _, _ in self.get_visible_monsters():
            if 0 <= dis <= REST_MONSTER_DISTANCE:
                yield False
        yield True
        if self.inventory.engraving_below_me.lower() != 'elbereth' and self.can_engrave() and \
                self.current_level().objects[self.blstats.y, self.blstats.x] not in \
                G.STAIR_UP | G.STAIR_DOWN | G.ALTAR | G.FOUNTAIN:
            self.engrave('Elbereth')
            return
        self.search(5)
    ...
    def _flee_downstairs_step(self):
        """ Next tile on the way to a nearby known down staircase when a mobile hostile is close, else None. """
        y0, x0 = self.blstats.y, self.blstats.x
        if not any(getattr(mon, 'mmove', 12) > 0 and max(abs(my - y0), abs(mx - x0)) <= FLEE_STAIRS_THREAT_DIST
                   for _, my, mx, mon, _ in self.get_visible_monsters()):
            return None
        dis = self.bfs()
        best = None
        for (sy, sx) in self.current_level().get_stairs(down=True):
            if self.current_level().objects[sy, sx] not in G.STAIR_DOWN:
                continue
            d = dis[sy, sx]
            if 0 < d <= FLEE_STAIRS_MAX_DIST and (best is None or d < dis[best]):
                best = (sy, sx)
        if best is None:
            return None
        path = self.path(y0, x0, *best, dis=dis)
        if len(path) < 2:
            return None
        ny, nx = path[1]
        if max(abs(ny - y0), abs(nx - x0)) != 1 or self.monster_tracker.monster_mask[ny, nx] or \
                not self.current_level().walkable[ny, nx]:
            return None
        return ny, nx
