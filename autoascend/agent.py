...
class Agent:
    ...
    def emergency_strategy(self):
        ...
        # hypothesis: fragile Healers get one-shot from a seemingly-safe HP because the emergency
        # prayer only fires at HP < max/5 (or /6) or HP < 6 -- so low that a single hard hit can drop
        # them from "safe" straight to dead, skipping the prayer window entirely. Raise the crisis
        # threshold to match the potion threshold (HP < max/3 or HP < 8) so prayer -- the reliable
        # full-heal backstop once potions are spent -- fires with real margin. This only changes
        # behaviour in near-death states (which are otherwise terminal), so it barely perturbs the
        # fixed-seed playout of healthy runs while converting some avoidable deaths into survival --
        # and more survival == more XP/depth == higher score across all four Healer identities.
        if (
