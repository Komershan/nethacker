...
class Agent:
    ...
    def cast(self, spell_name, direction):
        with self.atom_operation():
            ...
            def type_letters():
                ...
                if 'In what direction?' in self.message:
                    success[0] = True
                    # hypothesis: the direction was yielded as calc_direction's string, which step()
                    # sends as a raw key: 's' (south) means "self" at getdir (the Wizard bolts herself:
                    # "Boing!" off her own magic resistance), 'n'/'e'/'w' are "strange directions" that
                    # waste the Pw, and 'ne'/'sw'/... raise. So every Force bolt was lost while the
                    # monster kept biting. Send the real direction key and Wizards finally kill with it.
                    yield self._DIRECTION_ACTIONS[direction]
    ...
    def direction(self, y, x=None):
        ...
        action = self._DIRECTION_ACTIONS[dir]
