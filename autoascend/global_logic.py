...
# Which Dlvl 1 grind policy to use -- three independent lines of work reached this knob with
# different answers, so the merge keeps all three and picks one by held-out measurement:
#   'split' (Komershan, ryzen run): STRONG_OFFENSE_ROLES leave at GRIND_XL, the rest at HOSTILE_GRIND_XL
#   'all8'  (Komershan, m5 run): everyone grinds to ALL_GRIND_XL
#   'roles' (daglar-dragomirov@7a65e41): ROLE_GRIND_XL, with his per-role pick hunt, hunger exit
#           and Mines routes
GRIND_POLICY = 'all8'
...
class GlobalLogic:
    ...
    def _grind_xl(self):
        role = self.agent.character.role
        if GRIND_POLICY == 'all8':
            return ALL_GRIND_XL
        if GRIND_POLICY == 'roles':
            return ROLE_GRIND_XL.get(role, GRIND_XL)
        return GRIND_XL if role in STRONG_OFFENSE_ROLES else HOSTILE_GRIND_XL

    def _role_routes(self):
        return GRIND_POLICY != 'roles' or self.agent.character.role not in ROLES_WITHOUT_MINES_ROUTES

    def _pick_hunt_turns(self):
        if GRIND_POLICY != 'roles':
            return PICK_HUNT_TURNS
        return ROLE_PICK_HUNT_TURNS.get(self.agent.character.role, PICK_HUNT_TURNS)
    ...
    def _explore_levels_fully(self):
        # hypothesis: every level is fully explored before the bot takes stairs it already knows,
        # so the deep phase spends ~1000-2000 turns per Mines/dungeon level (traces: gnomes reach
        # only Dlvl 4-7 by turn 12000 and die there at Xp 6-8). Depth is worth far more than the
        # loot and experience that exploration yields (Dlvl 8 = 0.07 > Xp 7 = 0.05), so once past the
        # Dlvl 1 grind, head for the next level as soon as the way down is known. The Mines pick
        # hunt keeps full exploration -- meeting dwarves is its point. Only for gnomes and dwarves,
        # to whom the Mines are peaceful: other races diving the main dungeon measured far worse
        # without the loot and experience of full exploration (kni 0.12 -> 0.06, pri 0.11 -> 0.07).
        if self.milestone == Milestone.BE_ON_FIRST_LEVEL:
            return True
        return self.agent.character.race not in (Character.GNOME, Character.DWARF)
    ...
    def current_strategy(self):
        ...
        while 1:
            ...
            if self.milestone == Milestone.BE_ON_FIRST_LEVEL:
                ...
                # eat, move on down where corpses (and experience) come faster ('roles' policy only).
                grind_xl = self._grind_xl()
                condition = lambda: self.agent.blstats.experience_level >= grind_xl or \
                    (LEAVE_GRIND_WHEN_HUNGRY and GRIND_POLICY == 'roles' and
            ...
            (
                ...
                .preempt(self.agent, [
                    exploration_strategy(0).condition(lambda: self._explore_levels_fully() and not diving()),
